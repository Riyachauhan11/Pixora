...
const CropContent = () => {
  ...
  const [selectedImage, setSelectedImage] = useState(null); // the img being cropped
  const [isCropMode, setIsCropMode] = useState(false); // whether crop is active or not
  const [selectedRatio, setSelectedRatio] = useState(null); // currently selected aspect ratio
  const [cropRect, setCropRect] = useState(null); // blue crop rectangle overlay
  const [originalProps, setOriginalProps] = useState(null); // store og img properties for restoration
  ...
  const createCropRectangle = (image) => {
    ...
    const cropRectangle = new Rect({
      ...
      evented: true,
      name: "cropRect", // identifier for this rect
  ...
  const initializeCropMode = (image) => {
    ...
    // Make image non-selectable to prevent default tools
    image.set({
      selectable: false,
      evented: false,
    });
