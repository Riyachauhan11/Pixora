  ...
  const extractFilterValues = (imageObject) => {
    ...
    imageObject.filters.forEach((filter) => {
      ...
      if (config) {
        ...
        // Fabric stores values in normalized form (e.g., brightness 0.3 means +30%).
        if (config.key === "hue") {
          extractedValues[config.key] = Math.round(
            filterValue * (180 / Math.PI)
          );
        } else {
          // Multiply by 100 to convert normalized float to percentage.
          // Round to integer for slider use.
